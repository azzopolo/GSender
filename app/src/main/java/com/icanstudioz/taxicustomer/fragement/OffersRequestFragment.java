...
import com.icanstudioz.taxicustomer.adapter.OffersRequestAdapter;
...
public class OffersRequestFragment extends Fragment {
    ...
    public void getAcceptedRequest(String id, String status, String key) {
        ...
        String ride_id = getArguments().getString("ride_id");

        params.put("ride_id", ride_id);
        Server.setHeader(key);
        Server.get("api/user/myoffers/format/json?", params, new JsonHttpResponseHandler() {
            @Override
            public void onStart() {
                super.onStart();
            ...
            public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
                ...
                try {
                    ...
                    if (response.has("status") && response.getString("status").equalsIgnoreCase("success")) {
                        List<Offers> list = gson.fromJson(response.getJSONArray("data").toString(), new TypeToken<List<Offers>>() {
                        }.getType());
                        ...
                        if (response.has("data") && response.getJSONArray("data").length() == 0) {
                            ...
                            OffersRequestAdapter acceptedRequestAdapter = new OffersRequestAdapter(getActivity(), list);
